...
namespace RestFrames {
  ...
  void GFrame::SetGroup(Group* groupPtr){
    if(m_GroupPtr){
  ...
  GroupList* GFrame::GetListGroups() const {
    GroupList* groupsPtr = new GroupList();
    FillListGroupsRecursive(groupsPtr);
    return groupsPtr;
  }

  void GFrame::FillListGroupsRecursive(GroupList* groupsPtr) const{
    if(m_GroupPtr) groupsPtr->Add(m_GroupPtr);
    int Nchild = GetNChildren();
    for(int i = 0; i < Nchild; i++){
      GFrame *childPtr = dynamic_cast<GFrame*>(GetChildFrame(i));
      if(childPtr) childPtr->FillListGroupsRecursive(groupsPtr);
    }
  }

  bool GFrame::InitializeStates(const StateList* statesPtr){
    int Nchild = GetNChildren();
	...
	GFrame* rframePtr = dynamic_cast<GFrame*>(framesPtr->Get(f));
  ...
  bool GFrame::InitializeGroupStates(const GroupList* groupsPtr){
  ...
  bool GFrame::InitializeAnalysis(const StateList* statesPtr, const GroupList* groupsPtr){
    ClearStates();
    m_Mind = false;
    if(!m_Body){
  ...
  bool GFrame::InitializeAnalysisRecursive(const StateList* statesPtr, const GroupList* groupsPtr){
